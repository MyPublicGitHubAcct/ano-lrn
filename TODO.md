...
- ~~`notebooks/delay_line.ipynb`~~ — done (covers `delay_line` and `fractional_delay_line`)
...
Add `ping_pong_delay(signal, fs, delay_time, feedback, mix)` to `src/python/delay/ping_pong.py`. A ping-pong delay alternates echoes between left and right channels: the first echo appears on the opposite channel from the dry signal, the second returns to the same side, and so on. Internally, two fractional-delay lines (via `fractional_delay_line`) of length `delay_time * fs` samples are cross-fed — the output of the left delay feeds the input of the right delay and vice versa, each attenuated by `feedback`. The `mix` parameter (0–1) blends dry mono input with the wet stereo output; at `mix = 1` the output is fully wet. Return a `(t, L, R)` stereo tuple. Add tests verifying: at `feedback = 0` only one echo appears per channel and it lands at the correct fractional sample index, at `feedback = 0.5` a second echo appears in the opposite channel at `2 × delay_time`, and `mix = 0` returns the dry signal in both channels with equal amplitude. Add `plot_delay_ping_pong.py` showing a single click input with several bounces visible as alternating spikes in the L and R waveforms. Add `docs/delay/ping_pong.md`. Ping-pong delay is among the most commonly patched stereo effects in VCV Rack and a direct showcase of the fractional-delay and multitap infrastructure.
...
### Add multitap fractional delay line

Add `multitap_delay(signal, fs, taps)` to `src/python/delay/multitap.py`. A multitap delay reads from a single ring buffer at multiple fractional positions, mixing the results into a stereo output. The `taps` parameter is a list of `(delay_seconds, gain, pan)` tuples: `delay_seconds` is converted to a fractional sample count and resolved by `fractional_delay_line`, `gain` scales the tap amplitude, and `pan` ∈ [−1, 1] sets the left/right balance (using constant-power panning). All taps share one padded signal array; each tap calls `fractional_delay_line` with a scalar delay so there is no Python loop over samples. Return a `(t, L, R)` stereo tuple. Add tests verifying: a single tap at 0 seconds with `gain = 1` and `pan = 0` reproduces the dry signal in both channels, a tap at a delay of `d` seconds places energy at the correct sample index, and L/R power matches the expected constant-power panning law at `pan = ±1`. Add `plot_delay_multitap.py` showing a rhythmic eight-tap stereo echo pattern on an impulse train. Add `docs/delay/multitap.md`.
